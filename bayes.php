...
class Bayes
{
  ...
  function probRole($tinggi,$status)
  {
    $data = file_get_contents($this->pegawai);
    $hasil = json_decode($data,true);
    ...
    $t = 0;
    foreach ($hasil as $hasil) {
      if($hasil['Role'] == $tinggi && $hasil['status'] == $status){
        $t += 1;
      }else if($hasil['Role'] == $tinggi && $hasil['status'] == $status){
        $t +=1;
      }
    }
    return $t;
  ...
  function probItem($item,$status)
  {
    ...
    $t = 0;
    foreach ($hasil as $hasil) {
      if($hasil['Pemilihan_Item'] == $item && $hasil['status'] == $status){
        $t += 1;
      }else if($hasil['Pemilihan_Item'] == $item && $hasil['status'] == $status){
        $t +=1;
      }
    }
    return $t;
  ...
  function probMicro($micro,$status)
  {
    $data = file_get_contents($this->pegawai);
    $hasil = json_decode($data,true);
    ...
    $t = 0;
    foreach ($hasil as $hasil) {
      if($hasil['Micro_Macro'] == $micro && $hasil['status'] == $status){
        $t += 1;
      }else if($hasil['Micro_Macro'] == $micro && $hasil['status'] == $status){
        $t +=1;
      }
    }
    return $t;
